...
async def main():
    ...
    urls = urls[:1000]
    total_urls = len(urls)
    print(f"[Benchmark] Loaded {total_urls} URLs. Sending batch request to local API...")

    start_total = time.time()
    
    # Send a single POST request to the batch endpoint with a very long timeout (180s)
